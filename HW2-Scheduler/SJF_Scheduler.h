...
#ifndef Scheduler_SJF_Scheduler_h
#define Scheduler_SJF_Scheduler_h
...
public:
    /* schedule funtion **/
    void beginSchedule() {
        while (!processVector.empty() || !runningBuffer.empty() || !IOBuffer.empty() || !allProcess.empty()) {   //anything not empty, then continue
            ...
            for (vector<Process>::iterator i = processVector.begin(); i != processVector.end(); i++) {     // update CW of process in Ready queue
                i->setCW(i->getCW() + 1);
            }
    ...
    // before call this function, you should make sure processVector is not empty
    Process popShortestJob() {
        int minIndex = 0;
        Process minProcess = processVector[0];
        for (int i = 1; i < processVector.size(); i++) {
            if (processVector[i].getRemainingTime() < minProcess.getRemainingTime()) {
                minIndex = i;
                minProcess = processVector[i];
            }
        }
        processVector.erase(processVector.begin() + minIndex);  // delete this process from vector
        return minProcess;
    }
};
