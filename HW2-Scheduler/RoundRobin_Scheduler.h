...
#ifndef Scheduler_RoundRobin_Scheduler_h
#define Scheduler_RoundRobin_Scheduler_h
...
public:
    /* schedule funtion **/
    void beginSchedule() {
        while (!processQueue.empty() || !runningBuffer.empty() || !IOBuffer.empty() || !allProcess.empty()) {
            ...
            if (!runningBuffer.empty()) {
                ...
                if (!runningBuffer.empty() && TIME == p.getStateEnterTime() + quantum) {      // process run out of quantum
                    runningBuffer.clear();
                    processQueue.push_back(p);
                }
            ...
            if (runningBuffer.empty()) {
                if (!processQueue.empty()) {
                    ...
                    if (p.getCPUBurst() <= 0) {         // only when process use out CPU burst, then give a new one
